...
UniqueVisitsC <- visitsC %>% 
    group_by(Session) %>% 
    mutate(beforeSwap = max(Start),
           afterSwap = min(Start),
           sessionMinDay = min(CohortDay),
           sessionMaxDay = max(CohortDay),
           switchType = case_when(Start-afterSwap <= as.difftime(1, units = "days") ~ "afterswap",
                                  beforeSwap-Start <= as.difftime(1, units = "days") ~ "beforeswap"), #Making the 24 hours before a swap
          swapNumber = case_when(switchType == "beforeswap" ~ Session,
                                 switchType == "afterswap" ~ Session-1)
           )
...
visitsC <- left_join(visitsC, nosepokesC, by=c("Session","VisitID", "Start"))
...
cleanDataC <- UniqueVisitsC %>% 
  group_by(AnimalTag, CohortDay, VisitType, Probability, Fluid, Sex, Group) %>% 
...
learningCurveDS <- UniqueVisitsC %>%
  filter((afterSwap-Start <= as.difftime(1, units = "days") | 
          Start-beforeSwap <= as.difftime(1, units = "days")) & 
          VisitType != "Error!")

learningCurveDS <- learningCurveDS%>% 
  mutate(diffTime = if_else(CohortDay-sessionMinDay <= 1,
      floor(as.numeric(difftime(Start, afterSwap, units = "hours"))),
      floor(as.numeric(difftime(Start, beforeSwap, units = "hours")))))
```
...
#| fig-height: 16
    ...
    FluidVisitsS <- learningCurveDS %>% 

  filter(swapNumber != 0 & swapNumber != 17 & Fluid == "Sucrose") %>% 
  group_by(swapNumber, diffTime) %>% 
      mutate(totalVisits = n_distinct(VisitID)) %>% 
    filter(Probability == "Risky") %>% 
        mutate(visits = n_distinct(VisitID)/(totalVisits)) %>%
  ggplot(aes(x = diffTime, y = visits)) + #group = Group, fill = Group
  geom_line(linewidth = .5) +
  geom_point(shape = 21, color = "black") +
  ylab(label = "Ratio of Risky to Total Visits (Sucrose Only)") +
  xlab(label = "Hours Since Fluid Switch") +
  scale_y_continuous(limits = c(0,1)) +
  facet_wrap(vars(swapNumber))
    FluidVisitsS
ggsave(here("figures", "LearningCurve.pdf"))
```

```{r}
sucrose_data <- learningCurveDS %>% 
        filter(abs(diffTime) <= 24) %>%
  filter(!swapNumber %in% c(0, 15, 16, 17) & Fluid == "Sucrose") %>% 
  group_by(swapNumber, diffTime) %>% 
  summarise(
    totalVisits = n_distinct(VisitID),
    riskyVisits = n_distinct(VisitID[Probability == "Risky"]),
    visits = riskyVisits / totalVisits,
    .groups = "keep"
  ) %>%
  group_by(diffTime) %>% 
  summarize(meanVisits = mean(visits),
            n = n(),
            standardDeviation = sd(visits),
            SEM = standardDeviation/sqrt(n)) 
  # Only complete combinations where we know the swapNumber and diffTime exist in original data
  # complete(nesting(swapNumber, diffTime), fill = list(visits = NA)) %>%
  # filter(!is.na(visits))  # Remove the NA rows that complete() might have added

sucroseLearning <- sucrose_data %>%
  ggplot(aes(x = diffTime, y = meanVisits)) +
    geom_errorbar(aes(ymin = meanVisits-SEM,
                    ymax = meanVisits+SEM)) +
  geom_line(linewidth = .5) +
...
sucrose_data_end <- learningCurveDS %>% 
          filter(abs(diffTime) <= 24) %>%
  filter((swapNumber == 15 | swapNumber == 16 ) & Fluid == "Sucrose") %>% 
  group_by(swapNumber, diffTime) %>% 
  summarise(
    totalVisits = n_distinct(VisitID),
    riskyVisits = n_distinct(VisitID[Probability == "Risky"]),
  ...
  summarize(meanVisits = mean(visits),
            ...
            swapNumber) #%>%
  # Only complete combinations where we know the swapNumber and diffTime exist in original data
  # complete(nesting(diffTime), fill = list(visits = NA)) %>%
  # filter(!is.na(visits))  # Remove the NA rows that complete() might have added

sucroseLearningEnd <- sucrose_data_end %>%
  ggplot(aes(x = diffTime, y = meanVisits)) +
    geom_errorbar(aes(ymin = meanVisits-SEM,
                    ymax = meanVisits+SEM)) +
  geom_line(linewidth = .5) +
  geom_point(shape = 21, color = "black", fill = "yellow") +
  ylab(label = "Ratio of Risky to Total Visits (Sucrose After Probability Change)") +
  xlab(label = "Hours Since Fluid Switch") +
  scale_y_continuous(limits = c(0,1))
  # facet_wrap(vars(swapNumber))
...
water_data <- learningCurveDS %>% 
        filter(abs(diffTime) <= 24) %>%
  filter(!swapNumber %in% c(0, 15, 16, 17) & Fluid == "Water") %>% 
  group_by(swapNumber, diffTime) %>% 
  summarise(
    totalVisits = n_distinct(VisitID),
    riskyVisits = n_distinct(VisitID[Probability == "Risky"]),
...
waterLearning <- water_data %>%
  ggplot(aes(x = diffTime, y = meanVisits)) +
    geom_errorbar(aes(ymin = meanVisits-SEM,
                    ymax = meanVisits+SEM)) +
  geom_line(linewidth = .5) +
  geom_point(shape = 21, color = "black", fill = "yellow") +
  ylab(label = "Ratio of Risky to Total Visits (Water Only)") +
  xlab(label = "Hours Since Fluid Switch") +
  scale_y_continuous(limits = c(0,1))

water_data_end <- learningCurveDS %>% 
          filter(abs(diffTime) <= 24) %>%
  filter((swapNumber == 15 | swapNumber == 16 ) & Fluid == "Water") %>% 
  group_by(swapNumber, diffTime) %>% 
  summarise(
    totalVisits = n_distinct(VisitID),
    riskyVisits = n_distinct(VisitID[Probability == "Risky"]),
...
waterLearningEnd <- water_data_end %>%
  ggplot(aes(x = diffTime, y = meanVisits)) +
    geom_errorbar(aes(ymin = meanVisits-SEM,
                    ymax = meanVisits+SEM)) +
  geom_line(linewidth = .5) +
  geom_point(shape = 21, color = "black", fill = "yellow") +
  ylab(label = "Ratio of Risky to Total Visits (Water After Probability Change)") +
  xlab(label = "Hours Since Fluid Switch") +
  scale_y_continuous(limits = c(0,1))
  # facet_wrap(vars(swapNumber))
sucroseLearning + sucroseLearningEnd + waterLearning + waterLearningEnd
ggsave(here("figures", "learningCurveNew.pdf"), width = 16, height = 16)
```
...
learningCurveDS %>% 
  ...
  mutate(totalVisits = n_distinct(VisitID)) %>% 
    filter(Probability == "Risky") %>% 
        summarize(visits = n_distinct(VisitID)/(totalVisits)) %>% 
  gt()
# check_data <- learningCurveDS %>% 
#   filter(swapNumber != 0 & swapNumber != 17 & Fluid == "Sucrose") %>% 
#   group_by(diffTime) %>% 
#   mutate(totalVisits = n_distinct(VisitID, AnimalTag)) %>%
#   count(diffTime, swapNumber, Probability, totalVisits) %>% 
#   group_by(swapNumber, diffTime) %>% 
# print(.)
```
...
```{r}
      learningCurveDS %>% 
  group_by(diffTime, Group) %>% 
  # filter(Cage != 2) %>% 
      summarize(totalVisits = n_distinct(VisitID, AnimalTag),
                Group, Corner, VisitID, AnimalTag) %>% 
        group_by(diffTime, Group, Corner) %>% 
        summarize(cornerRatio = n_distinct(VisitID, AnimalTag)/totalVisits,
                  n_distinct(VisitID, AnimalTag),
                  totalVisits) %>% 
  unique()
```
...
cleanDataCSucrose <- cleanDataCSucrose %>% 
  ...
  group_by(AnimalTag, Day, VisitType) %>% 
  mutate(VisitTypeVisits = sum(Choices), VTPreference = VisitTypeVisits/Visits) %>% 
  group_by(Day) %>% 
      mutate(Mice = n_distinct(AnimalTag)) %>% 
    group_by(Day, Group) %>%
      mutate(MiceGroup = n_distinct(AnimalTag))
...
UniqueVisitsC %>%
  ...
  summarise(start = min(as.Date(Start)),
          ...
          UniqueDays = n_distinct(CohortDay), 
          .groups = "keep"
        	) %>%
  gt() %>%
...
RiskYMax <- UniqueVisitsC %>% 
  group_by(AnimalTag, CohortDay, VisitType, Probability) %>% 
...
p1 <- UniqueVisitsC %>% # Sucrose
  ...
  summarize(visits = n_distinct(Session, VisitID),
            .groups = "keep") %>% 
      ggplot(aes(x = CohortDay, y = visits, fill = VisitType, group = AnimalTag)) + #This sets up the x and y axis variables
...
UniqueVisitsC %>%
  ...
  group_by(AnimalTag, CohortDay, Probability, Fluid) %>% 
  ...
  summarize(visits = n_distinct(Session, VisitID),
      ...
      ggplot(aes(x = CohortDay, y = visits, fill = Fluid, group = AnimalTag)) + #This sets up the x and y axis variables
  geom_line(aes(group = AnimalTag), color = "darkgray") +
  geom_point(shape = 21, size = 2, color = "black") +
...
UniqueVisitsC %>%
  ...
  summarize(visits = n_distinct(Session, VisitID),
      ...
      ggplot(aes(x = CohortDay, y = licks, fill = Fluid, group = AnimalTag)) + #This sets up the x and y axis variables
  geom_line(aes(group = AnimalTag), color = "darkgray") +
  geom_point(shape = 21, size = 2, color = "black") +
  geom_vline(xintercept = c(6.5, 10.5, 14.5, 20.5), color = "gray", linetype = "dashed") +
  scale_x_continuous(name = "Experiment Day",
...
UniqueVisitsC %>%
  group_by(AnimalTag, CohortDay, Probability, Group) %>% 
  
  filter(VisitType != "Error!", Fluid == "Sucrose") %>% 
  summarize(visits = n_distinct(Session, VisitID),
            licks = sum(LickNumber),
            .groups = "keep") %>% 
  ggplot(aes(x = CohortDay, y = visits, fill = Group, group = AnimalTag)) + #This sets up the x and y axis variables
  # geom_line(aes(group = AnimalTag), color = "darkgray") +
  geom_point(shape = 21, size = 2, color = "black") +
...
SEBC <- visitsC %>%
  ...
  filter(VisitType != "Error!" & Fluid == "Sucrose")  %>%
  group_by(AnimalTag, CohortDay, Group) %>%
  summarize(sVisits = n(), .groups = "drop") %>%
  group_by(CohortDay, Group) %>%
  summarize(groupAvgVisits = mean(sVisits, na.rm = T),
      ...
      ggplot(aes(x = CohortDay, y = groupAvgVisits, fill = Group, group = Group)) + #This sets up the x and y axis variables
...
control <- UniqueVisitsC %>%
  ...
  summarize(UniqueVisitID = n_distinct(Session, VisitID),
            .groups = "keep") %>% 
      ggplot(aes(x = CohortDay, y = UniqueVisitID, fill = VisitType, group = AnimalTag)) + #This sets up the x and y axis variables
...
LiquidConsumption <- UniqueVisitsC %>%
  ...
  group_by(AnimalName, CohortDay, Fluid, Group) %>%
  filter(AnimalName != "19774") %>%
  summarize(licks = LickNumber) %>%
  ggplot(aes(fill = Fluid, y = licks, x = CohortDay)) +
...
## (Broken) Group Visit Type Bar Chart
```{r}
#| fig-width: 16
#| fig-height: 12
...
SucroseRiskRatio <- UniqueVisitsC %>% 
  ...
  summarize(Visits = n(),
            ...
            .groups = "keep") %>% 
  ggplot(aes(x = CohortDay, y = yVal, fill = Group, color = Group, group = Group)) +
...
WaterRiskRatio <- UniqueVisitsC %>% 
  ...
  summarize(Visits = n(),
            ...
            yVal = RiskyVisits/(SafeVisits+RiskyVisits),
            .groups = "keep") %>% 
  ggplot(aes(x = CohortDay, y = yVal, color = Group, fill = Group, group = Group)) +
  geom_line(aes(group = Group), color = "grey")+
  geom_point()+
    geom_vline(xintercept = c(6.5, 10.5, 14.5, 20.5), color = "gray", linetype = "dashed")+
...
UniqueVisitsC %>%
  ...
  ggplot(aes(x = CohortDay, y = RiskyProb, color = Group, fill = Group)) +
...
UniqueVisitsC %>% 
  filter(Group == "control" | Group == "auto HET") %>%
  group_by(CohortDay, Group, AnimalTag) %>%
  ...
  summarize(Sucrose_Ratio = mean(SucroseRatio, na.rm = T),
            ...
            SRSE = SRSD/sqrt(n()),
            .groups = "keep") %>% 
ggplot(aes(x = CohortDay, y = Sucrose_Ratio, group = Group, fill = Group,  color = Group)) +
  geom_point() +
  geom_line() +
  geom_errorbar(aes(
...
## (Unfinished) Learning Curve Graph
```{r}
LearningCurveDS <- LearningCurveDS %>% 
