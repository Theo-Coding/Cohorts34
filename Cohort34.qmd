...
sucrose_data <- learningCurveDS %>% 
        filter(abs(diffTime) <= 24 & Group != "control") %>%
  filter(!swapNumber %in% c(0, 15, 16, 17) & Fluid == "Sucrose") %>% 
  group_by(swapNumber, diffTime, Group) %>% 
  summarise(
...
sucroseLearning <- sucrose_data %>%
  ggplot(aes(x = diffTime, y = meanVisits, fill = Group)) +
    # geom_errorbar(aes(ymin = meanVisits-SEM,
    #                 ymax = meanVisits+SEM)) +
  geom_line(linewidth = .5) +
  geom_point(shape = 21,  size = 2, color = "black") +
  ylab(label = "Ratio of Risky to Total Visits") +
  xlab(label = "Hours Since Fluid Switch (Sucrose)") +
  scale_y_continuous(limits = c(0,1))
...
waterLearning <- water_data %>%
  ...
  geom_point(shape = 21, size = 2, color = "black") +
  ylab(label = "Ratio of Risky to Total Visits") +
  xlab(label = "Hours Since Fluid Switch (Water)") +
  scale_y_continuous(limits = c(0,1))
...
ggsave(here("figures", "learningCurveNotControlNoErrorBar.pdf"), width = 16, height = 16)
```
