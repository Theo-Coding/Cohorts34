...
learningCurveDS <- UniqueVisitsC %>%
  filter((Start-afterSwap <= as.difftime(1, units = "days") | 
          beforeSwap-Start <= as.difftime(1, units = "days")) & 
          VisitType != "Error!")
          # !(CohortDay == 1 & Session == 1)) &
learningCurveDS %>% 
  group_by(Session)
```
...
learningCurveDS <- learningCurveDS%>% 
  mutate(diffTime = if_else(CohortDay-sessionMinDay <= 1,
      floor(as.numeric(difftime(afterSwap, Start, units = "hours"))),
      floor(as.numeric(difftime(beforeSwap, Start, units = "hours")))))
      
      learningCurveDS %>% 
        group_by(diffTime, AnimalTag, Corner) %>% 
        summarize(licks = n_distinct(VisitID)) %>% 
          gt()
...
```{r}
  ...
  group_by(diffTime, Group) %>% 
      summarize(totalVisits = n_distinct(VisitID, AnimalTag),
                Group, Probability, VisitID, AnimalTag, Session) %>% 
        group_by(diffTime, Group, Probability) %>% 
    filter(Probability == "Risky") %>% 
        summarize(visits = n_distinct(VisitID, AnimalTag)/totalVisits,
                  n_distinct(VisitID, AnimalTag),
                  totalVisits, Session) %>% 
  ggplot(aes(x = diffTime, y = visits, group = Group, fill = Group, color = Group)) +
  scale_y_continuous(limits = c(0,1)) +
  geom_point() +
  ylab(label = "Probability Ratio") +
  xlab(label = "Hours Since Fluid Switch (Risky)") +
  facet_wrap(vars(Session))

    FluidVisitsW <- learningCurveDS %>% 
  group_by(diffTime, Group) %>% 
      summarize(totalVisits = n_distinct(VisitID, AnimalTag),
                Group, Probability, VisitID, AnimalTag, Session) %>% 
        group_by(diffTime, Group, Probability) %>% 
        filter(Probability == "Safe") %>% 
        summarize(visits = n_distinct(VisitID, AnimalTag)/totalVisits,
                  n_distinct(VisitID, AnimalTag),
                  totalVisits, Session) %>% 
  ggplot(aes(x = diffTime, y = visits, group = Group, fill = Group, color = Group)) +
