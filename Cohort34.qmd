...
UniqueVisitsC %>%
  ...
  filter(VisitType != "Error!", Fluid == "Sucrose", (ExperimentDay == 10 | ExperimentDay == 14 | ExperimentDay == 6 | ExperimentDay == 20 | ExperimentDay == 24) & Session < 6) %>% 
  summarize(visits = n_distinct(Session, VisitID),
            licks = sum(LickNumber),
            .groups = "keep") %>% 
...
AddingVisits <- cleanDataC %>% 
  group_by(AnimalTag, Day) %>% 
  summarize(Visits = sum(Choices), .groups = "drop")
cleanDataC <- left_join(cleanDataC, AddingVisits, by=c("AnimalTag", "Day"))

cleanDataC <- cleanDataC %>% 
  mutate(ChoiceFrequency = Choices/Visits)
                   
#VisitType Variables ------
VTVisits <- cleanDataC %>% 
  group_by(AnimalTag, Day, VisitType) %>% 
  summarize(VisitTypeVisits = sum(Choices), .groups = "keep") 
cleanDataC <- left_join(cleanDataC, VTVisits)

    VTPreference <- cleanDataC %>% 
    group_by(AnimalTag, Day, VisitType) %>% 
      summarize(VTPreference = VisitTypeVisits/Visits)
    cleanDataC <- left_join(cleanDataC, VTPreference, relationship = "many-to-many")
              
#Probability and Fluid sorted variables
PFPreferenceBind <- cleanDataC %>%      
  group_by(AnimalTag, Day, Probability, Fluid) %>% 
    mutate(sampleSize = sum(Choices),
           PFPreference = Choices/sampleSize)
  cleanDataC <- left_join(cleanDataC, PFPreferenceBind, relationship = "many-to-many")
  
  PFvisits <- cleanDataC %>% 
    group_by(AnimalTag, Day, Probability, Fluid) %>% 
    mutate(PFVisits = sum(Choices)) 
    cleanDataC <- left_join(cleanDataC, PFvisits)
    
#Mouse Variables -------
MiceCounter <- cleanDataC %>% 
      group_by(Day) %>% 
      summarize(Mice = n_distinct(AnimalTag))
    cleanDataC <- left_join(cleanDataC, MiceCounter)
    
    MiceCounterGroup <- cleanDataC %>%
      group_by(Day, Group) %>%
      summarize(MiceGroup = n_distinct(AnimalTag))
    cleanDataC <- left_join(cleanDataC, MiceCounterGroup)

    cleanDataC <- unique(cleanDataC)
```

```{r}
#Analysis for sucrose only ------------------------------------------
AddingVisits <- cleanDataCSucrose %>% 
  group_by(AnimalTag, Day) %>% 
  summarize(Visits = sum(Choices), .groups = "drop")
  cleanDataCSucrose <- left_join(cleanDataCSucrose, AddingVisits)

AddingRiskChoices <- cleanDataCSucrose %>%
  group_by(AnimalTag, Day, Probability) %>%
  summarize(RiskChoices = sum(Choices), .groups = "keep")
cleanDataCSucrose <- left_join(cleanDataCSucrose, cleanDataCSucroseTemp) %>% 
mutate(RiskPreference = RiskChoices/Visits*100)

#VisitType Variables ------
VTVisits <- cleanDataCSucrose %>% 
  group_by(AnimalTag, Day, VisitType) %>% 
  summarize(VisitTypeVisits = sum(Choices), .groups = "keep") 
cleanDataCSucrose <- left_join(cleanDataCSucrose, VTVisits)

    VTPreference <- cleanDataCSucrose %>% 
    group_by(AnimalTag, Day, VisitType) %>% 
      summarize(VTPreference = VisitTypeVisits/Visits, .groups = "keep")
    cleanDataCSucrose <- left_join(cleanDataCSucrose, VTPreference)
    
    
#Mouse Variables -------
    MiceCounter <- cleanDataCSucrose %>% 
      group_by(Day) %>% 
      summarize(Mice = n_distinct(AnimalTag), .groups = "keep")
    cleanDataCSucrose <- left_join(cleanDataCSucrose, MiceCounter)
        MiceCounterGroup <- cleanDataCSucrose %>%
      group_by(Day, Group) %>%
      summarize(MiceGroup = n_distinct(AnimalTag), .groups = "keep")
    cleanDataCSucrose <- left_join(cleanDataCSucrose, MiceCounterGroup)
  cleanDataCSucrose <- unique(cleanDataCSucrose)
...
cleanDataC %>% #Get visits per mouse per type,
  ...
  summarize(PerMouse = sum(Choices)/sum(PFVisits),
            Mice,
            .groups = "drop") %>% 
  group_by(Fluid, Probability, Day, AnimalTag) %>% 
#   summarize(MouseTot = sum(PerMouse)) %>% 
# filter(MouseTot <= 1)
  group_by(Fluid, Probability, Day, VisitType) %>%
  summarize(yVal = sum(PerMouse)/Mice) %>% 
...
VTGroupBars <- cleanDataCSucrose %>% #Get visits per mouse per type
  ...
  summarize(yVal = sum(VTPreference)/MiceGroup/2)
...
```{r}
# UniqueVisitsC %>%
#   group_by(Group, Fluid, Corner, Session) %>%
#   filter(ExperimentDay == max(ExperimentDay)) %>%
#   ggplot(aes(x = ExperimentDay, y = , group = Group, fill = Group,  color = Group)) +
#   geom_point() +
#   geom_line() +
#   facet_grid(Corner ~ Fluid)
```
